...
public class ContinuousKiekerReadingTest extends LibredeTest {
	...
	private String resultsfile = "/home/torsten/Schreibtisch/jettytests/local/1l_600s_500t_ubuntu_visits/info/results.txt";
	private String configfile = "/home/torsten/git/librede/tools.descartes.librede.estimation.tests/src/tools/descartes/librede/datasource/kieker/kiekercsv.librede";
	...
	private class RunnerThread extends Thread {
		...
		public void run() {
			...
			// the next time stamp when librede should calculate resource
			// demands
			...
			while (!stop) {
				...
				// more precise: give the watchthread time to push the initil
				// data to the dataSourceListener
				...
				if (!stop && System.currentTimeMillis() >= nextexecutiontimestamp) {
					...
					/*
					 * for (Class<? extends IEstimationApproach> approach :
					 * results.getApproaches()) { double responsetimeerror =
					 * results.getApproachResponseTimeError(approach); double
					 * utilizationerror =
					 * results.getApproachUtilizationError(approach); }
					 */
					try {
	...
	public void runEstimation() {
		...
		conf = null;
		conf = Librede.loadConfiguration(new File(configfile).toPath());
		conf.getEstimation().setWindow(1);
		fixTimeStamps(conf);
		try {
			LibredeVariables var = new LibredeVariables(conf);
	...
	public void test() {
		...
		/*
		 * Quantity<Time> defaulttime =
		 * conf.getEstimation().getStartTimestamp(); Unit<Time> unit =
		 * defaulttime.getUnit(); double actualtime = System.nanoTime();
		 * Unit<Time> nanotime = Time.NANOSECONDS;
		...
		Librede.initDataSources(var, existingDatasources, dataSourceListener);
		// sleep some time to give the repo time to initialize
		// more precise: give the watchthread time to push the initil data to
		...
		while (!stop) {
			LibredeResults results = Librede.executeContinuousOnline(5000, var, existingDatasources,
					dataSourceListener);
