		...
		if (aggregation == Aggregation.AVERAGE) {
			...
			if (entity instanceof Service) {
				...
				for (ExternalCall curCall : calls) {
					double rt = repository.aggregate(StandardMetrics.RESPONSE_TIME, Time.SECONDS, curCall, aggregation,
							start, end);
					double tput = repository.aggregate(StandardMetrics.THROUGHPUT, RequestRate.REQ_PER_SECOND, curCall,
			...
			if (sumTput > 0.0) {
				return unit.convertFrom(sumRt / sumTput, Time.SECONDS);
			} else {
				return Double.NaN;
			}
		}
