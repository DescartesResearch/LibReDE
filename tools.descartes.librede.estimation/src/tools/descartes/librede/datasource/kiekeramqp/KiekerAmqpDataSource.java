...
public class KiekerAmqpDataSource extends AbstractDataSource {
	...
	@ParameterDefinition(name = "MaxTimeSec", label = "Maximum elapsed time before we try to trigger events.", required = false, defaultValue = "20")
	...
	@ParameterDefinition(name = "WaitTimeSec", label = "Time we wait if we did not have enough entries yet", required = false, defaultValue = "2")
	private int wait;
	...
	public void load() {
		this.analysisThread = new KiekerAnalysisThread(this, 
				uri, queueName, triggercount, mincount, maxtimesec, wait, filedirectory+"kieker/");
	...
	private void saveToCsv(Metric<?> metric, String entityname, ArrayList<Double> values, ArrayList<Double> timestamps) {
		...
		String foldername = filedirectory+"csv/";
		...
		//actual --> 30 mins
		if(actualtime-lastCsvPrintTimeStamp>1800000){
			long lasttimetmp = lastCsvPrintTimeStamp;
			//update timestamp
			lastCsvPrintTimeStamp = actualtime;
			//create new folder
			File folder = new File(foldername+""+lastCsvPrintTimeStamp+"/");
			folder.mkdir();
			if(lasttimetmp!=-1){
				new CopyThread(foldername+""+lasttimetmp, foldername+"root/").start();
			}
		}
		filename = filename+"_"+entityname+".csv";
		filename = foldername+""+lastCsvPrintTimeStamp+"/"+filename;
