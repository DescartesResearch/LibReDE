...
import net.descartesresearch.librede.configuration.editor.forms.details.MeasurementDataDetailsPage;
import net.descartesresearch.librede.configuration.impl.TraceConfigurationImpl;
...
public class MeasurementDataMasterBlock extends AbstractMasterBlockWithButtons {
	...
	protected void registerPages(DetailsPart part) {
		MeasurementDataDetailsPage details = new MeasurementDataDetailsPage(page, domain, model);
		part.registerPage(TraceConfigurationImpl.class, details);
	}
	...
	private class FilteredList extends ComputedList {
		...
		@Override
		protected List<?> calculate() {
			List<TraceConfiguration> list = new ArrayList<TraceConfiguration>();
			for (TraceConfiguration current : model.getInput().getObservations()) {
	...
	private class MeasurementTreeObservableFactory implements IObservableFactory {
		...
		public IObservable createObservable(Object target) {
			...
			} else if (target instanceof TraceConfiguration) {
				return null;
			} else {
	...
	private class MeasurementTreeStructureAdvisor extends TreeStructureAdvisor {
		...
		public Boolean hasChildren(Object element) {
			if (element instanceof TraceConfiguration) {
				return false;
	...
	private void addMeasurementTraces(Object[] results) {
		for (Object r : results) {
			TraceConfiguration series = ConfigurationFactory.eINSTANCE.createTraceConfiguration();
	...
	protected Control createItemsList(Composite parent) {
		...
		IObservableList l2 = EMFEditProperties.value(domain, ConfigurationPackage.Literals.TRACE_CONFIGURATION__METRIC).observeDetail(l);
